...
export function SideBar({ onSidebarToggle }) {
    ...
    const handleTitleSave = async (boardId) => {
        try {
            ...
            if (updatedTitle) {
                // Ensure we're passing an object with both `boardId` and `title`
                const updatedBoardData = { _id: boardId, title: updatedTitle }
    
                await saveBoard(updatedBoardData)
                await loadBoards()
                closeModal()  // Ensure modal closes after save
            }
        } catch (err) {
