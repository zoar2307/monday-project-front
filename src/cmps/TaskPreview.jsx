...
import { updateGroup, updateMemberUnreadTaskUpdates, updateTask } from '../store/actions/board.actions'
...
export function TaskPreview({ idx, task, group, board }) {
    ...
    useEffect(() => {
        ...
        setMember(board.members.find(member => member._id === user._id))
    }, [task])
    ...
    function onOpenConversation() {
        ...
        if (member.unread) {
            updateMemberUnreadTaskUpdates(task.id)
        }
    }
                ...
                >
                    ...
                    <div className='task-title'>
                        ...
                        <div className='task-title-txt'>
                            ...
                            <div onClick={onOpenConversation} className={`chat ${member &&
                                member.unread && member.unread.find(unReadTask => unReadTask.tId === task.id) ? 'unseen' : ''}`}  >
                                ...
                                {member &&
                                    member.unread &&
                                    member.unread.map(unReadTask => {
                                        if (unReadTask.tId === task.id) {
                                            return < div className='message-count'>{unReadTask.count}</div>
                                        }
